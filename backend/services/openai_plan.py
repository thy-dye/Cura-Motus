...
_STEP_PREFIX_PATTERN = re.compile(r"^Step:\d+\s*", re.IGNORECASE)


def _clean_instructions(instructions):
    '''Strips ExerciseDB's raw "Step:1 " style prefix, we already render our own step numbers.'''
    if not instructions:
        return instructions
    return [_STEP_PREFIX_PATTERN.sub("", step) for step in instructions]
...
def _title_case(text):
    '''ExerciseDB names come back all lowercase ("wrist circles") — capitalize each word for display.'''
    if not text:
        return text
    return _WORD_PATTERN.sub(lambda m: m.group(0).capitalize(), text)
    ...
    for step in plan.get("plan", []):
        ...
        if source == "locked" and exercise_id in LOCKED_EXERCISES:
            ...
            validated_steps.append({
                **step,
                "name": _title_case(catalog_entry.get("name")),
                "gif_url": catalog_entry.get("gifUrl"),
                "instructions": _clean_instructions(catalog_entry.get("instructions")),
            })
...
@openai_plan_bp.route("/api/resolve-exercise", methods=["POST"])
def resolve_exercise_route():
    body = request.get_json(silent=True) or {}
    name = body.get("name", "").strip()

    if not name:
        return jsonify({"error": "missing_field", "detail": "name is required"}), 400

    try:
        match = resolve_exercise(name)
    except RuntimeError as e:
        return jsonify({"error": "openai_not_configured", "detail": str(e)}), 502
    except Exception as e:
        return jsonify({"error": "resolve_failed", "detail": str(e)}), 502
    ...
    return jsonify({
        "match": {
            "exerciseId": match.get("exerciseId"),
            "name": _title_case(match.get("name")),
            "gifUrl": match.get("gifUrl"),
            "instructions": _clean_instructions(match.get("instructions")),
        }
    }), 200
